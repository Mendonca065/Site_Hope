...
export function VideoSobre() {
  ...
  return (
    <section className="relative z-10 w-full bg-[#FFC700] py-16 md:py-32 overflow-x-clip flex flex-col items-center rounded-b-[40px] md:rounded-b-[80px] border-b-[8px] border-black">
      
      {/* Background Layers */}
      <div 
        className="absolute inset-0 z-0 opacity-100 bg-no-repeat bg-[length:115%_auto] bg-[position:right_-30px_bottom_150px] md:bg-[length:50%_auto] md:bg-[position:right_bottom]"
        style={{ backgroundImage: `url(${videoSobre2})` }}
      />
      <div 
        className="absolute inset-0 z-0 bg-no-repeat bg-[length:120%_auto] bg-[position:left_top_-50px] md:bg-[length:60%_auto] md:bg-[position:left_top_-110px]"
        style={{ backgroundImage: `url(${videoSobre1})` }}
      />
      
      {/* ========================================== */}
      {/* RETÂNGULO LARANJA DECORATIVO DA DIREITA    */}
      {/* Posição: top-[385px] (distância do topo), -right-[180px] (escondido para a direita) */}
      {/* Tamanho: w-[413px] (largura), h-[1919px] (altura) */}
      {/* Arredondamento: rounded-t-[74px] (só arredonda o topo) */}
      {/* ========================================== */}
      <div className="absolute top-[385px] -right-[180px] w-[413px] h-[1420px] bg-[#FF6B2B] rounded-t-[74px] rounded-b-[74px] z-0 hidden lg:block"></div>
      ...
      <div className="relative z-10 w-full max-w-7xl px-4 md:px-8 flex flex-col items-center">
        ...
        <div className="w-full max-w-5xl aspect-video bg-black rounded-[2rem] md:rounded-[3rem] overflow-hidden border-4 border-black shadow-[8px_8px_0px_rgba(0,0,0,1)] relative flex items-center justify-center mb-24 md:mb-72">
          ...
          <video 
            ref={videoRef}
            className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-300 ${stage === 'playing' ? 'opacity-100 z-30' : 'opacity-0 z-0'}`}
        ...
        <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 mb-32 items-center">
          ...
          <div className="flex flex-col">
            <img 
              ...
              className="w-full max-w-[500px] mb-8 object-contain drop-shadow-xl"
            />
            ...
            <div className="font-['Montserrat',_sans-serif] font-medium text-lg md:text-2xl lg:text-[34px] leading-[1.31] text-justify text-black space-y-6 lg:max-w-[772px]">
              <p>Bem-vindo à 9ª edição do movimento que transforma uma geração.</p>
              <p>Desde 2018, o Hangar 3 é um ambiente seguro em João Pessoa para adolescentes enfrentarem tabus, superarem ansiedades e descobrirem quem realmente são em Deus.</p>
              <p>Vivemos dias em que a esperança parece distante, mas nossa convicção permanece inabalável: Jesus é a nossa única esperança e, porque Ele vive, hoje nós temos vida.</p>
            </div>
          </div>

          {/* Coluna da Direita: Grid de Fotos */}
          <div className="grid grid-cols-2 gap-4 md:gap-6">
            <img src={foto1} alt="Foto 1" className="w-full h-auto object-contain drop-shadow-lg" />
            <img src={foto2} alt="Foto 2" className="w-full h-auto object-contain drop-shadow-lg" />
            <img src={foto3} alt="Foto 3" className="w-full h-auto object-contain drop-shadow-lg" />
            <img src={foto4} alt="Foto 4" className="w-full h-auto object-contain drop-shadow-lg" />
            <img src={foto5} alt="Foto 5" className="w-full h-auto object-contain drop-shadow-lg" />
            <img src={foto6} alt="Foto 6" className="w-full h-auto object-contain drop-shadow-lg" />
          </div>
